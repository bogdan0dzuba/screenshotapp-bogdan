...
Каждый тег `v*` запускает GitHub Actions, проверяет проект, собирает Universal ZIP и публикует подписанный `appcast.xml`. Приложение проверяет этот канал через Sparkle, скачивает архив из GitHub Releases, сверяет EdDSA-подпись и предлагает перезапуск. Автоматическую проверку и скачивание можно отключить в настройках.
