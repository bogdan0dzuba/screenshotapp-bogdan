...
[![Версия](https://img.shields.io/badge/версия-v0.5.5-7B61FF)](https://github.com/bogdan0dzuba/screenshotapp-bogdan/releases/tag/v0.5.5)
...
GitHub Actions проверяет проект и Universal-сборку при каждом обновлении `main`. Релиз публикуется одной командой `./script/publish_release.sh 0.5.5`: закрытый EdDSA-ключ берется из macOS Keychain, локально подписывает `appcast.xml` и никогда не передается GitHub. Приложение проверяет этот канал через Sparkle, скачивает архив из GitHub Releases, сверяет подпись и предлагает перезапуск. Автоматическая проверка и скачивание включены по умолчанию; их можно отключить в настройках.
