...
[![Версия](https://img.shields.io/badge/версия-v5.52-7B61FF)](https://github.com/bogdan0dzuba/screenshotapp-bogdan/releases/tag/v5.52)
...
Если общий доступ к записи экрана отсутствует или не применился, приложение открывает системный выбор экрана/окна macOS. Выбор дает доступ только к выбранному содержимому на время захвата: переустановка и сброс разрешений не нужны. Для области сначала выберите экран, затем выделите нужный фрагмент. Для прокрутки выберите конкретное окно: панели скриншотера не входят в этот источник.

Этот путь использует кадр из `SCStream`: одиночный API `SCScreenshotManager` на тестовой системе возвращал отказ `−3801` даже после выбора источника. Завершение, отмена и ошибки закрывают сессию; при отказе нет перехода к захвату всего экрана через старую команду. Обычный захват с уже действующим общим разрешением сохраняется.
...
5. Нажмите захват и выберите экран или окно в системном диалоге macOS. Общий доступ «Запись экрана» нужен только для захвата без системного выбора источника.
