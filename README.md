...
[![Версия](https://img.shields.io/badge/версия-v0.5.15-7B61FF)](https://github.com/bogdan0dzuba/screenshotapp-bogdan/releases/tag/v0.5.15)
...
GitHub Actions проверяет проект и Universal-сборку при каждом обновлении `main`. Релиз публикуется одной командой `./script/publish_release.sh 0.5.15`: закрытый EdDSA-ключ берется из macOS Keychain, локально подписывает `appcast.xml` и никогда не передается GitHub. Приложение проверяет этот канал через Sparkle при каждом запуске и затем раз в сутки. Обычное обновление автоматически загружается, проверяется, устанавливается и перезапускает приложение; ручная проверка остается в настройках.
