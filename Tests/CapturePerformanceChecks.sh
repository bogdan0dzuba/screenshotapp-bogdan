...
require_text "$MODEL" "regionSelectionController.selectRegion()" "area hotkey does not show the in-process selector"
require_text "$MODEL" "captureService.capture(rect: rect" "selected area is not handed to the background capture process"
