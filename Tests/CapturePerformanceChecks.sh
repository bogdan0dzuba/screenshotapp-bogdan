...
require_text "$MODEL" "case .area: captureWithSystemUI(mode)" "area hotkey does not start the native macOS selector immediately"
reject_text "$MODEL" "private func captureArea()" "area hotkey still freezes a full display before showing the selector"
require_text "$CAPTURE_SERVICE" 'case .area: arguments += ["-i", "-s"]' "area capture does not request the native crosshair selector"
