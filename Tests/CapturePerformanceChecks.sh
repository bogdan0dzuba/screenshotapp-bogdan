...
REVISION_INDEX="Sources/ScreenshotCore/Stores/HistoryImageRevisionIndex.swift"
...
require_file "$REVISION_INDEX" "per-capture preview revision index is missing"
...
require_text "$HISTORY" "func imageRevision(for item: CaptureItem) -> Int" "history does not expose per-capture preview revisions"
reject_text "$HISTORY" "@Published private(set) var imageRevision = 0" "history still invalidates every preview with one global revision"
require_text "$HISTORY" "scheduleRetentionCleanup" "retention cleanup is still on the capture presentation path"
require_text "$HISTORY" "Task.detached(priority: .utility)" "retention cleanup is not scheduled off the main actor"
...
require_text "$MODEL" "case .area: captureArea()" "area hotkey still starts the legacy screencapture selector"
require_text "$MODEL" "private func captureArea()" "ScreenCaptureKit-backed area capture is missing"
...
require_text "$CAPTURE_SERVICE" "func capture(_ prepared: PreparedScrollCapture) async throws -> CGImage" "scroll capture still forces every frame through a temporary PNG"
require_text "$CAPTURE_SERVICE" "return image" "scroll capture does not expose the native CGImage to the classifier"
...
require_text "$SHELF" "history.imageRevision(for: item)" "shelf previews do not use per-capture revisions"
reject_text "$SHELF" "revision: history.imageRevision," "shelf still passes the global history revision"
reject_text "$SHELF" "Task.sleep(for: .milliseconds(100))" "shelf preview still waits before every decode"
require_text "$LOADER" "private actor CaptureImageDecodeQueue" "preview decodes are not serialized"
require_text "$LOADER" "CaptureImageCacheKey" "preview decodes have no shared cache key"
require_text "$LOADER" "cached" "preview loader has no shared decoded-image cache"
