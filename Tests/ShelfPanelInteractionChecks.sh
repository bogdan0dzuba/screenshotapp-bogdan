...
require_view "reduceTransparency ? 0.78 : 0.24" "native glass keeps the nearly opaque black scrim"
...
reject_view 'Label("С прокруткой"' "bottom capture bar still wastes space on text"
require_view 'captureButton("С прокруткой", "arrow.up.and.down.text.horizontal")' "scrolling capture lost its icon-only control"
...
require_view 'Text(model.statusMessage)' "copy, OCR and save feedback disappeared from the shelf"
require_view '.font(.caption2)' "status feedback still consumes excessive vertical space"
...
require_view '.glassEffect(.clear, in: shape)' "expanded shelf does not use the more transparent Clear Liquid Glass"
require_view 'fallbackBackground(shape: shape, tintOpacity: 0.28)' "fallback expanded glass is still too opaque"
