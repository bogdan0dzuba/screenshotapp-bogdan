...
require_source "acceptsMouseMovedEvents = true" "shelf panel does not refresh pointer state while hovering"
...
require_view '.help("Очистить историю и удалить файлы")' "history-delete icon has no hover explanation"
...
require_view "ScreenshotDragSurface(url: item.imageURL" "history rows do not expose the full screenshot row as a drag surface"
require_view "ScreenshotDragSurfaceView" "history screenshot drag surface has no native AppKit view"
require_view "beginDraggingSession(with:" "history screenshot drag does not start a native dragging session"
require_view "NSDraggingSource" "history screenshot drag source has no AppKit drag contract"
require_view "ScreenshotTransfer.pasteboardItem(for:" "history drag does not provide the stored PNG file"
require_view "shouldClickOnRelease" "history drag surface does not preserve a plain selection click"
require_view "addCursorRect(bounds, cursor: .arrow)" "shelf views do not force a visible arrow cursor"
require_view "NSCursor.arrow.set()" "shelf does not restore the pointer after the resize cursor"
